...
background.src = "https://wallpaper.wiki/wp-content/uploads/2017/04/wallpaper.wiki-Free-Download-1080p-Space-Background-PIC-WPD008677.jpg";
...
function Diamond(x, y, w, h, exists, size){
  ...
  this.color =['#4BB74C','#517B58','#5B9C64'];
  this.size = size;
}
  ...
  for(var j = 0; j < diamonds.length; j++) {
    if(diamonds[j].exists ) {
      ...
      if (distance < this.size + diamonds[j].size) {
        ...
        evil.color = "Green";
        count++;
        para.textContent = 'Diamond count: ' + count;
      }
    }
    if (count == 10)
    {
...
function YouWon(){
  ...
  ctx.fillStyle = "Green";
  ctx.textAlign = "center";
  ctx.fillText("Þú vannst", canvas.width/2, canvas.height/2); 
  ctx.delete()
}
